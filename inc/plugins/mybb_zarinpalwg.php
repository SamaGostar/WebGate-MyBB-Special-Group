...
function mybb_zarinpalwg_install()
{
...
با سپاس از عضویت شما.',
	...
	$db->write_query("CREATE TABLE `".TABLE_PREFIX."myzp` (
	 ...
	 function mybb_zarinpalwg_is_installed()
{
	global $db;
		return $db->table_exists("myzp");
}

function mybb_zarinpalwg_uninstall()
{
global $db;
...
function mybb_zarinpalwg_deactivate()	
{
global $db;
...
function myzpzg_plugin_admin_cp($sub_menu)
{
	global $mybb, $lang;
		...
		$sub_menu[$key] = array('id' => 'zarinpalwg', 'title' => "بسته های عضویت آنی (زرین پال)", 'link' => "index.php?module=user-zarinpalwg");
     return $sub_menu;
}

function myzpzg_handle($action)
{
	$action['zarinpalwg'] = array('active' => 'zarinpalwg', 'file' => 'zarinpalwg.php');
	return $action;
}
