...
$page->add_breadcrumb_item("پرداخت آنلاين زرين پال", "index.php?module=user-zarinpalwg");
...
if($mybb->input['action'] == "edit_pack")
{
	$sub_tabs['edit_pack'] = array(
		'title' => 'ویرایش بسته',
		'link' => "index.php?module=user-zarinpalwg&amp;action=edit_pack",
	...
	$sub_tabs['manual_add'] = array(
		...
		'link' => "index.php?module=user-zarinpalwg&amp;action=manual_add",
	...
	$sub_tabs['tractions'] = array(
		...
		'link' => "index.php?module=user-zarinpalwg&amp;action=tractions",
...
$d = "سال";
		...
		$pname=$over['title'];
		$table->construct_cell("<strong><a href=\"index.php?module=user-zarinpalwg&amp;action=edit_pack&amp;num={$over['num']}\">{$over['title']}</a></strong></td><td>{$over['description']}</td><td>{$over['period']} {$d}  </td><td>{$over['price']} تومان</td>");
		$popup = new PopupMenu("sub_{$over['num']}", $lang->options);
		$popup->add_item($lang->delete, "index.php?module=user-zarinpalwg&amp;action=delete_pack&amp;num={$over['num']}");
		$popup->add_item($lang->edit, "index.php?module=user-zarinpalwg&amp;action=edit_pack&amp;num={$over['num']}");
	...
	$pagination = draw_admin_pagination($mybb->input['page'], $per_page, $total_rows, "index.php?module=user-zarinpalwg&amp;page={page}");
	echo $pagination;					
...
$db->insert_query("myzp", $insert_query);
					...
					admin_redirect("index.php?module=user-zarinpalwg");
					}
}
...
// Time
		...
		$form = new Form("index.php?module=user-zarinpalwg&amp;action=add_pack", "post");
		if($errors)
	{
...
//
	...
	$form = new Form("index.php?module=user-zarinpalwg&amp;action=edit_pack&amp;num={$num}", "post");
			if($errors)
	{
		$page->output_inline_error($errors);
	}
	...
	if($mybb->input['action'] == "tractions")
{
	...
	if($mybb->input['uname'])
	{
		...
		if(!$user['uid'])
		{
			...
			admin_redirect("index.php?module=user-zarinpalwg&action=tractions");
		}
...
if($track['stauts'] != 0)
{
	...
	$pagination = draw_admin_pagination($mybb->input['page'], $per_page, $total_rows, "index.php?module=user-zarinpalwg&amp;action=tractions&amp;page={page}{$additional_criteria}");
				echo $pagination;
				echo "<div class = \"float_left\"><a href=\"index.php?module=user-zarinpalwg&amp;action=deletealltrac\">حذف تمامی تراکنش‌های پایان یافته</a></div>";
	...
	$neper['1']="قبل";
    $neper['2']="بعد";
	$form = new Form("index.php?module=user-zarinpalwg&amp;action=tractions", "post");
...
//Message
	...
	$form = new Form("index.php?module=user-zarinpalwg&amp;action=manual_add", "post");
		if($errors)
	{
		$page->output_inline_error($errors);
	}
